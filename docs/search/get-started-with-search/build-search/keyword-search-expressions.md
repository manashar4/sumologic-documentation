...
Sumo Logic normalizes the way it matches raw messages to log query source expressions independent of the tier that the data resides in. One of the impacts of the normalization shows up in the behaviour where consecutive whitespace characters within quoted phrases are replaced with a single space character. This ensures consistency in query results when searching data in the continuous/frequent/infrequent tiers.
